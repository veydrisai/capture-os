...
export const clientGoLive = task({
    ...
    run: async (payload) => {
        ...
        if (email) {
            await sendEmail(email, `🚀 You're live! Welcome to RevenueCS`, `<!DOCTYPE html><html><body style="font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif;background:#0d1a0d;margin:0;padding:40px 20px;">
<div style="max-width:560px;margin:0 auto;background:#111f11;border-radius:16px;overflow:hidden;border:1px solid rgba(168,255,71,0.15);">
  ...
  <div style="padding:24px 40px;">
    <p style="color:rgba(255,255,255,0.4);font-size:12px;text-align:center;margin:0;">Go-live: ${goLiveDate} · Monthly retainer: $${monthlyRetainer ?? 0}/mo</p>
        ...
        // 2. Internal alert
        await sendEmail(internalEmail, `🚀 Client LIVE — ${businessName}`, `<p style="font-family:sans-serif;"><strong>${businessName}</strong> is now live!<br>Email: ${email ?? "—"}<br>System: ${systemType ?? "—"}<br>Retainer: $${monthlyRetainer ?? 0}/mo<br>ROI Dashboard: ${roiDashboardUrl ?? "not set"}<br>Go-live: ${goLiveDate}</p>`);
    ...
    run: async () => {
        ...
        for (const client of liveClients) {
            if (!client.email)
                continue;
            try {
                await sendEmail(client.email, `📊 Your weekly revenue capture report — ${client.businessName}`, `<!DOCTYPE html><html><body style="font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif;background:#0d1a0d;margin:0;padding:40px 20px;">
<div style="max-width:560px;margin:0 auto;background:#111f11;border-radius:16px;overflow:hidden;border:1px solid rgba(168,255,71,0.15);">
