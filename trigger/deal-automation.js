...
export const demoDoneAgreementSender = task({
    ...
    run: async (payload) => {
        ...
        if (contactEmail && agreementUrl) {
            await sendEmail(contactEmail, `Your agreement is ready — ${dealTitle}`, `<!DOCTYPE html><html><body style="font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif;background:#f4f4f5;margin:0;padding:40px 20px;">
<div style="max-width:560px;margin:0 auto;background:#fff;border-radius:12px;overflow:hidden;box-shadow:0 2px 12px rgba(0,0,0,0.08);">
  ...
  <div style="padding:32px 40px;">
    ...
    <p style="margin-top:28px;font-size:12px;color:#9ca3af;">Questions? Reply to this email or reach us at michael@revenuecs.com</p>
        ...
        // 2. Internal alert
        await sendEmail(internalEmail, `✅ Demo done — agreement sent to ${contactName ?? contactEmail ?? "unknown"}`, `<p style="font-family:sans-serif;"><strong>Deal:</strong> ${dealTitle}<br><strong>Contact:</strong> ${contactName} (${contactEmail})<br><strong>System:</strong> ${systemType} | Setup: $${setupFee} | Monthly: $${monthlyRetainer}/mo<br><strong>Agreement URL:</strong> ${agreementUrl ?? "not set"}</p>`);
...
export const agreementSignedOnboarding = task({
    ...
    run: async (payload) => {
        ...
        if (contactEmail && intakeFormUrl) {
            await sendEmail(contactEmail, `Welcome! Next step: complete your onboarding intake`, `<!DOCTYPE html><html><body style="font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif;background:#f4f4f5;margin:0;padding:40px 20px;">
<div style="max-width:560px;margin:0 auto;background:#fff;border-radius:12px;overflow:hidden;box-shadow:0 2px 12px rgba(0,0,0,0.08);">
  ...
  <div style="padding:32px 40px;">
    ...
    <p style="margin-top:28px;font-size:12px;color:#9ca3af;">Setup fee: $${setupFee ?? 0} | Monthly retainer: $${monthlyRetainer ?? 0}/mo</p>
        ...
        // 2. Internal alert
        await sendEmail(internalEmail, `🎉 Agreement signed — onboarding started for ${contactName ?? contactEmail ?? "unknown"}`, `<p style="font-family:sans-serif;"><strong>Deal:</strong> ${dealTitle}<br><strong>Contact:</strong> ${contactName} (${contactEmail})<br><strong>System:</strong> ${systemType} | Setup: $${setupFee} | Monthly: $${monthlyRetainer}/mo<br><strong>Intake form:</strong> ${intakeFormUrl ?? "not set"}<br><strong>Signed at:</strong> ${payload.agreementSignedAt ?? new Date().toISOString()}</p>`);
        return { ok: true, contactEmail };
    },
});
...
export const demoBookedAlert = task({
    ...
    run: async (payload) => {
        ...
        await sendEmail(internalEmail, `📅 Demo booked — ${contactName ?? contactEmail ?? "unknown"}`, `<p style="font-family:sans-serif;"><strong>Deal:</strong> ${dealTitle}<br><strong>Contact:</strong> ${contactName} (${contactEmail})<br><strong>Booked at:</strong> ${demoBookedAt}</p>`);
        return { ok: true };
    },
});
