...
var demoDoneAgreementSender = task({
...
var agreementSignedOnboarding = task({
...
<div style="max-width:560px;margin:0 auto;background:#fff;border-radius:12px;overflow:hidden;box-shadow:0 2px 12px rgba(0,0,0,0.08);">
  ...
  <div style="padding:32px 40px;">
    ...
    await sendEmail(
      ...
      `<p style="font-family:sans-serif;"><strong>Deal:</strong> ${dealTitle}<br><strong>Contact:</strong> ${contactName} (${contactEmail})<br><strong>System:</strong> ${systemType} | Setup: $${setupFee} | Monthly: $${monthlyRetainer}/mo<br><strong>Intake form:</strong> ${intakeFormUrl ?? "not set"}<br><strong>Signed at:</strong> ${payload.agreementSignedAt ?? (/* @__PURE__ */ new Date()).toISOString()}</p>`
    );
    return { ok: true, contactEmail };
  }, "run")
});
var demoBookedAlert = task({
...
export {
  agreementSignedOnboarding,
  demoBookedAlert,
  demoDoneAgreementSender
};
//# sourceMappingURL=deal-automation.mjs.map
