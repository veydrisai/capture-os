...
<div style="max-width:560px;margin:0 auto;background:#111f11;border-radius:16px;overflow:hidden;border:1px solid rgba(168,255,71,0.15);">
  ...
  <div style="background:rgba(168,255,71,0.05);border-top:1px solid rgba(168,255,71,0.1);border-bottom:1px solid rgba(168,255,71,0.1);padding:24px 40px;text-align:center;">
    ...
    ${roiDashboardUrl ? `<a href="${roiDashboardUrl}" style="display:inline-block;background:#a8ff47;color:#0d1a0d;padding:14px 36px;border-radius:10px;text-decoration:none;font-weight:700;font-size:14px;">View Your ROI Dashboard →</a>` : `<p style="color:rgba(255,255,255,0.4);font-size:13px;">Your dashboard will be shared shortly.</p>`}
  </div>
...
var weeklyRoiEmails = schedules_exports.task({
  id: "weekly-roi-emails",
  cron: "0 15 * * 1",
  run: /* @__PURE__ */ __name(async () => {
    const liveClients = await db.select().from(clients).where(eq(clients.onboardingStatus, "live"));
    const sent = [];
    const failed = [];
...
<div style="max-width:560px;margin:0 auto;background:#111f11;border-radius:16px;overflow:hidden;border:1px solid rgba(168,255,71,0.15);">
  ...
  <div style="background:rgba(255,255,255,0.03);border-top:1px solid rgba(255,255,255,0.06);padding:24px 40px;">
    ...
    ${client.roiDashboardUrl ? `<a href="${client.roiDashboardUrl}" style="display:inline-block;background:#a8ff47;color:#0d1a0d;padding:12px 28px;border-radius:10px;text-decoration:none;font-weight:700;font-size:13px;">View Full Dashboard →</a>` : ""}
  </div>
...
//# sourceMappingURL=client-live.mjs.map
