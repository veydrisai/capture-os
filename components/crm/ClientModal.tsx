...
export default function ClientModal({ client, onClose, onSaved }: Props) {
  ...
  return (
    <div style={overlay} onClick={(e) => { if (e.target === e.currentTarget) onClose(); }}>
      <div style={{ ...panel, maxWidth: 620, maxHeight: "92vh", overflowY: "auto" }}>
        <div style={header}>
          <div>
            <h2 style={title}>{client ? "Edit Client" : "New Client"}</h2>
            <p style={subtitle}>{client ? `Onboarding: ${completedCount}/${CHECKLIST.length} steps` : "Add to onboarding"}</p>
          </div>
